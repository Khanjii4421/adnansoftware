...
const Orders = () => {
  ...
  const fetchOrders = async (retryCount = 0) => {
    ...
    try {
      ...
      console.log('[Orders] Attempt:', retryCount + 1, 'of', MAX_RETRIES + 1);
      ...
      console.log(`[Orders] Response received in ${fetchTime}ms`);
      console.log('Orders response:', response.data);
      
      const ordersData = response.data.orders || [];
      // Use total from backend, or fallback to orders length
      const totalCount = response.data.total !== undefined && response.data.total !== null 
        ? response.data.total 
        : ordersData.length;
      const processingTime = response.data.processing_time_ms || fetchTime;
      
      console.log(`[Orders] Received ${ordersData.length} orders (Total: ${totalCount}) in ${processingTime}ms`);
      
      // Set total orders count and fetch time
      setTotalOrders(totalCount);
      setFetchTime(processingTime);
      
      // Show success message for large datasets
      if (ordersData.length > 1000) {
        console.log(`[Orders] Large dataset loaded: ${ordersData.length} orders`);
      }
      
      // Debug: Check shipper_price values (only for first few orders to avoid console spam)
      if (ordersData.length > 0 && ordersData.length <= 10) {
      ...
      if (
        ...
        console.log(`[Orders] Timeout error, retrying in ${RETRY_DELAY}ms... (${retryCount + 1}/${MAX_RETRIES})`);
      ...
      if (
        ...
        console.log(`[Orders] Network error, retrying in ${RETRY_DELAY}ms... (${retryCount + 1}/${MAX_RETRIES})`);
        await new Promise(resolve => setTimeout(resolve, RETRY_DELAY));
        return fetchOrders(retryCount + 1);
      }
  ...
  const startCameraScan = async () => {
    try {
      ...
      const scannerId = 'return-scan-camera';
      ...
      if (cameras && cameras.length > 0) {
        ...
        await qrCodeScanner.start(
          ...
          (errorMessage) => {
            ...
            // Only show error if it's a permission or camera error
        ...
        setScanError('No camera found. Please use a device with a camera.');
        setIsScanning(false);
      }
    } catch (error) {
    ...
    try {
      if (qrCodeScannerRef.current) {
        ...
        try {
          ...
          console.log('Scanner stop:', stopError.message);
        }
        // Clear the scanner
        try {
          await scanner.clear();
          ...
          // Ignore clear errors
          console.log('Scanner clear:', clearError.message);
        }
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-3">
                  <p className="text-sm font-medium text-gray-700">
                    Showing <span className="font-bold text-indigo-600">{orders.length}</span> orders
                    {totalOrders > orders.length && ` of ${totalOrders} total`}
                  </p>
                  {fetchTime > 0 && (
                    <p className="text-xs text-gray-500">
                      Loaded in {(fetchTime / 1000).toFixed(2)}s
                    </p>
                  )}
                </div>
                {orders.length > 1000 && (
              ...
              <div className="mb-4">
                {!isScanning ? (
                  <button
                    ...
                    className="w-full px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2 mb-2"
                  >
                    <span>📷</span>
                  ...
                  <div className="space-y-2">
                    <div id="return-scan-camera" className="w-full rounded-lg overflow-hidden bg-black" style={{ minHeight: '250px' }}></div>
                    <button
                      type="button"
